---
name: hyatt-strategy
description: Hyatt-first hotel strategy. Use when a destination may have useful Hyatt options, when comparing Hyatt cash vs points, or when checking if Chase UR should be transferred to Hyatt.
version: 1.1.0
---

# Hyatt Strategy

Hyatt is often one of the strongest hotel-program angles in points travel.
...
1. If account-specific context is needed, log into Hyatt first.
   - Retrieve credentials via your normal secrets manager or runtime injection flow rather than storing them in plaintext env files.
   - Use `hyatt_login` with verified credentials.
...
- the property is a premium Hyatt brand
...
- the tool data looks scraped or uncertain and needs confirmation on Hyatt's site
...
- A successful home-IP or residential-IP check does not prove the browser path is viable if the browser fingerprint still shows obvious automation signals like `HeadlessChrome`, `navigator.webdriver = true`, zero plugins, or missing `window.chrome`.
...
- If using a patched local Hyatt MCP, point your MCP config to that server explicitly rather than assuming the default package has the improvements you need.
- A patched Hyatt MCP can support external browser handoff via `HYATT_BROWSER_CDP_URL` (preferred) or `BROWSER_CDP_URL`. When one of those env vars is set, Hyatt MCP can connect to that browser over Chrome DevTools Protocol instead of launching its own raw local Playwright Chromium. This is the clean path for using a stealthier persistent browser backend.
- After changing MCP server config or browser env for Hyatt, remember many agent runtimes load MCP servers at startup, so the running chat may not use the patched server until the agent restarts.
...
- A stronger workflow for date-finding is:
  ...
  3. prefer the strongest available final-verification environment if server-side verification remains blocked
- For date-finding tasks, a good reusable workflow is: find the MaxMyPoint property page via DuckDuckGo (`site:maxmypoint.com <property>`), use that for candidate windows, then confirm those windows on Hyatt. Do not represent MaxMyPoint alone as final bookable truth.
